// ══════════════════════════════════════════════════════
//  FRÍO CARS — clientes.js
//  Tablas: cliente + vehiculo (Supabase/backend)
//
//  cliente:  id_cliente, numero_documento, nombre,
//            apellido, telefono, correo, direccion
//
//  vehiculo: id_vehiculo, placa, marca, modelo,
//            tipo_vehiculo, anio, id_cliente (FK)
// ══════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════
//  CARGAR CLIENTES (con sus vehículos)
// ══════════════════════════════════════════════════════
async function cargarClientes() {
  ...
  try {
    // Cargamos clientes y vehículos en paralelo
    ...
    tbody.innerHTML = `
      <tr><td colspan="7">
        <div class="empty-state">
          <span>⚠️</span>
          <p style="font-weight:600;color:#dc2626">Error al cargar clientes</p>
          <button onclick="cargarClientes()" style="margin-top:.8rem;background:var(--pri);color:#fff;border:none;border-radius:10px;padding:.5rem 1.2rem;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:.82rem">↺ Reintentar</button>
        </div>
      </td></tr>`;
  }
}
...
function renderTabla(clientes) {
  ...
  clientes.forEach(c => {
    ...
    const veh       = c.vehiculos?.[0]; // vehículo principal
...
// ══════════════════════════════════════════════════════
//  FORMULARIO — toggle / cancelar / limpiar
// ══════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════
//  GUARDAR CLIENTE (POST o PUT) + VEHÍCULO
// ══════════════════════════════════════════════════════
async function guardarCliente() {
  ...
  if (!documento) { mostrarToast("El número de documento es obligatorio", "warn"); return; }
  if (!telefono)  { mostrarToast("El teléfono es obligatorio", "warn"); return; }
  ...
  try {
    ...
    const bodyCliente = { nombre, apellido, numero_documento: documento, telefono, correo, direccion };
    ...
    // ── 2. Guardar vehículo si hay placa ─────────────
    if (placa && marca && modelo) {
      const bodyVeh = { placa, marca, modelo, tipo_vehiculo, anio: anio ? parseInt(anio) : null, id_cliente };
      await fetch(`${API}/vehiculos`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(bodyVeh)
      });
      // Si falla el vehículo no bloqueamos — el cliente ya quedó guardado
    }
    ...
    ? (c.vehiculos||[]).map(v => `
        <div style="background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:.8rem 1rem;margin-top:.6rem">
          ...
          <div style="font-size:.75rem;color:var(--muted);margin-top:2px">
            Placa: <strong style="color:var(--pri)">${(v.placa||"—").toUpperCase()}</strong>
  ...
  overlay.innerHTML = `
    <div class="modal-box">
      <div style="display:flex;align-items:center;gap:.8rem;margin-bottom:1.2rem">
        ...
        <div>
          <h3 style="margin:0;font-size:1rem">${c.nombre} ${c.apellido||""}</h3>
          <p style="font-size:.72rem;color:var(--muted);margin:0">Ficha de cliente · ID ${c.id_cliente}</p>
        </div>
      </div>
      ...
      <div style="margin-top:1.2rem;display:flex;gap:.6rem;justify-content:flex-end">
        <button onclick="this.closest('.modal-overlay').remove()"
          style="background:var(--bg);border:1.5px solid var(--border);color:var(--muted);border-radius:10px;padding:.5rem 1.1rem;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:.84rem">
          Cerrar
        </button>
        <button onclick="this.closest('.modal-overlay').remove();editarCliente(${id})"
          style="background:var(--pri);color:#fff;border:none;border-radius:10px;padding:.5rem 1.1rem;font-weight:700;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:.84rem">
          ✏️ Editar
        </button>
      </div>
    </div>`;
  ...
  return new Promise(resolve => {
    ...
    overlay.innerHTML = `
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:1.8rem;max-width:360px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,.15)">
        <p style="font-size:.9rem;font-weight:600;color:var(--text);margin-bottom:1.4rem;line-height:1.6;white-space:pre-line">${mensaje}</p>
